...
// 功能描述：    一个二次封装了的OPC UA库，支持从opc ua服务器读写节点数据，批量读写，订阅，批量订阅，历史数据读取，方法调用操作。
                详情参考OPCFoundation在Github上的代码.
    ...
    public class OpcUaClient
    {
        ...
        public OpcUaClient()
        {
            ...
            var configuration = new ApplicationConfiguration {
                ...
                ApplicationUri = $"urn:{Environment.MachineName.ToLower()}:RootCloud:OPCUAClient", //Kepp this syntax
                ProductUri = "http://rootcloud.com/RootLink/DC/Protocol/OPCUAClient/",
                ...
                SecurityConfiguration = new SecurityConfiguration {
                    ...
                    ApplicationCertificate = new CertificateIdentifier {
                        ...
                        StorePath = "CurrentUser\\RootCloud",
            ...
            // 创建证书
            ApplicationInstance app = new ApplicationInstance(configuration);
            bool certOk = app.CheckApplicationInstanceCertificate(true, 0).Result;
        ...
        public async Task ConnectServer(string serverUrl,
            MessageSecurityMode securityMode = MessageSecurityMode.None, string securityPolicy = SecurityPolicies.None)
        {
            m_session = await Connect(serverUrl, securityMode, securityPolicy);
        }
            ...
            MessageSecurityMode securityMode = MessageSecurityMode.None, string securityPolicy = SecurityPolicies.None)
        {
            ...
            var url = GetDiscoveryUrl(serverUrl);
            ...
            EndpointDescription endpointDescription = CoreClientUtils.SelectEndpoint(url, endpoints, UseSecurity, securityMode, securityPolicy);
        ...
        public bool WriteNodes(string[] tags, object[] values, out bool[] status, out string[] error)
        {
            WriteValueCollection valuesToWrite = new WriteValueCollection();
            ...
            bool result = true;
            status = new bool[results.Count];
            error = new string[results.Count];
            for (int i = 0; i < results.Count; i++)
            {
                if (StatusCode.IsGood(results[i]))
                {
                    status[i] = true;
                    error[i] = string.Empty;
                }
                else
                {
                    status[i] = false;
                    error[i] = $"code:{results[i].Code}  msg:{results[i].ToString()}";
        ...
        public void AddSubscription(string key, string[] tags, Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback)
        {
            ...
            {
                list.Add(new NodeId(tag));
            }

            AddSubscription(key, list.ToArray(), callback);
        }

        /// <summary>
        /// 新增一个订阅，需要指定订阅的关键字，订阅的NodeId，以及回调方法
        /// </summary>
        /// <param name="key">关键字</param>
        /// <param name="nodeId">tag</param>
        /// <param name="callback">回调方法</param>
        public void AddSubscription(string key, NodeId nodeId, Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback)
        {
            AddSubscription(key, new NodeId[] { nodeId }, callback);
        }

        /// <summary>
        /// 新增一批订阅，需要指定订阅的关键字，订阅的NodeId数组，以及回调方法
        /// </summary>
        /// <param name="key">关键字</param>
        /// <param name="nodeIds">节点名称数组</param>
        /// <param name="callback">回调方法</param>
        /// <param name="samplingInterval">采样周期，默认100ms</param>
        /// <returns>返回对应ClienHandle与NodeId的对应关系</returns>
        public Dictionary<uint, NodeId> AddSubscription(string key, NodeId[] nodeIds, Action<string, MonitoredItem, MonitoredItemNotificationEventArgs> callback, int samplingInterval = 100)
        {
            Dictionary<uint, NodeId> dict = new Dictionary<uint, NodeId>();
            ...
            for (int i = 0; i < nodeIds.Length; i++)
            {
                var item = new MonitoredItem {
                    ...
                    AttributeId = Attributes.Value,
                    DisplayName = nodeIds[i].ToString(),
                ...
                dict.Add(item.ClientHandle, item.StartNodeId);
            }
